...
def decodehttoken(input):
        # The obfuscation function has changed.
        # To generate ttoken:
        # thanks go to https://github.com/assistyoubram for providing the logic in javascript!!
        # // get below from the IMG tag contained on "http://192.168.2.254/login.htm"
...
class ExperiaBoxV10ADeviceScanner(DeviceScanner):
    ...
    def _update_info(self):
        ...
        # _LOGGER.warning(httoken_search)
        # authenticity_token = base64.b64decode(httoken_search.group(1)).decode('utf-8')
        # _LOGGER.warning(httoken_search.group(1))
        authenticity_token = decodehttoken(httoken_search.group(1))

        # _LOGGER.debug(httoken_search)
        _LOGGER.debug(authenticity_token)
        ...
        index_authenticity_token = decodehttoken(index_httoken_search.group(1))
